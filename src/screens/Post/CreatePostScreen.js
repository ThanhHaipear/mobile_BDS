...
export default function CreatePostScreen({ navigation }) {
    ...
    useFocusEffect(
        useCallback(() => {
            ...
            return undefined;
        }, [mode, hasDraft, hasPickedLocation, resetForm])
    );
    ...
    const prefillFromPost = useCallback((p) => {
        ...
        if (Array.isArray(p.images) && p.images.length) {
            const mapped = p.images
                .map((it) => it?.image_url || it?.url)
                .filter(Boolean)
                .map((u) => ({ uri: String(u), __remote: true }));
            setAssets(mapped);
        } else {
            setAssets([]);
        }
    ...
    }, [mode, postId, initialPost, prefillFromPost]);

    // Khi đổi postId/mode => cho phép prefill lại
    useEffect(() => {
        didPrefillRef.current = false;
        if (mode === 'edit') fetchDetailIfNeeded();
    ...
    const submit = async () => {
        ...
        try {
            ...
            if (mode === 'edit' && postId) {
                ...
                await client.patch(ENDPOINTS.POST_DETAIL(postId), payload);
